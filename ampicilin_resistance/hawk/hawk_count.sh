...
do
  ...
  ${JELLY_DIR}/jellyfish count -C -o ${OUTPREFIX}_kmers/tmp -m ${KMER_SIZE} -t ${CORES} -s ${MEM} <(zcat ${sample}_1.fasta.gz ${sample}_2.fastq.gz)
  ...
  then
    ...
    ${JELLY_DIR}/jellyfih merge -o ${OUTPREFIX}_kmers_jellyfish ${OUTPREFIX}_kmers/tmp*
  fi
  ...
  then
    ...
    awk -f ${HAWK_DIR}/countTotalKmer.awk ${OUTPREFIX}.kmers.hist.csv >> ${DIFF_DIR}/total_kmer_counts.txt
